...
async function enqueuePasswordResetEmail(userEmail, portal="patient") {
  const otp = generateOTP();
  const job = await emailQueue.add('sendPasswordReset', { userEmail, otp, portal }, {
    attempts: 5,
    backoff: { type: 'exponential', delay: 1000 },
    removeOnComplete: true,
  });
  const waitingCount = await emailQueue.getWaitingCount();
  console.log(`Enqueued password reset email job ${job.id} for ${userEmail}`);
  return {
    jobId: job.id,
    position: waitingCount,
    expectedArrivalSeconds: waitingCount * (Number(process.env.EMAIL_DELAY) || 1),
...
function buildEmailTemplate(job_name, userEmail, otp) {
  ...
  } else if (job_name === "sendPasswordReset") {
    subject = "Reset Your MDSystem Password";
    htmlContent = passwordResetTemplate(otp); // ✅ callable fn
  } else {
...
module.exports = { enqueueEmail, enqueueEmailVerification, enqueueEmail2FA, enqueuePasswordResetEmail, buildEmailTemplate };

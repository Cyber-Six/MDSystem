...
const Auth = () => {
  ...
  return (
    <div className="relative min-h-screen overflow-hidden">
      ...
      {/* Sliding Login Panel */}
      <div className={`auth-panel fixed top-0 h-screen w-full max-w-[420px] bg-white dark:bg-neutral-900 shadow-2xl z-10 overflow-y-auto transition-all duration-400 ease-out ${isPanelOpen && activeView === 'login' ? 'right-0' : '-right-full'}`}>
      ...
      {activeView === 'register' && (
        <div className="fixed inset-0 flex items-center justify-center z-20 px-4 py-8">
          ...
          {/* Modal Card */}
          <div className="relative w-full max-w-md bg-white dark:bg-neutral-900 rounded-2xl shadow-2xl overflow-y-auto max-h-[90vh]">
            <div className="px-8 md:px-6 sm:px-5 py-8 md:py-6">
              {/* Close Button */}
              <button
                onClick={() => handleViewChange('login')}
                className="absolute top-4 right-4 p-2 hover:bg-neutral-100 dark:hover:bg-neutral-800 rounded-full transition-colors z-30"
              ...
              <div className="text-center mb-8">
                ...
                />
                <h2 className="text-2xl font-bold text-secondary-900 dark:text-white font-heading mb-1">
                  Create Account
                </h2>
                <p className="text-xs text-neutral-600 dark:text-neutral-400">

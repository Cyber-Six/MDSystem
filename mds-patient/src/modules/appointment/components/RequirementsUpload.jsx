...
import { stageFile, unstageFile } from '../patient-appointment-service';
...
const RequirementsUpload = ({ requirements, uploadedFiles, onFileUpload, onNext, onBack }) => {
  ...
  const handleFileChange = async (reqId, file) => {
    ...
    try {
      // Unstage previous file if one was already uploaded for this requirement
      const prevFileId = uploadedFiles[reqId]?.fileId;
      if (prevFileId) {
        try { await unstageFile(prevFileId); } catch { /* non-fatal */ }
      }

      const fileId = await stageFile(file);
      onFileUpload(reqId, { fileId, fileName: file.name });
    } catch (err) {
      setUploadErrors((prev) => ({ ...prev, [reqId]: err.message || 'Upload failed' }));
    } finally {
  ...
  return (
    <div className="bg-white dark:bg-neutral-900 rounded-lg shadow-lg p-6">
      ...
      <div className="space-y-4">
        {requirements.map((req) => (
          <div key={req.id} className="p-4 border border-neutral-200 dark:border-neutral-700 rounded-lg">
            ...
            />
            {uploading[req.id] && (
              <p className="mt-1 text-xs text-primary-600 dark:text-primary-400">Uploading...</p>
            )}
            {uploadErrors[req.id] && (
              <p className="mt-1 text-xs text-red-600 dark:text-red-400">{uploadErrors[req.id]}</p>
            )}
            {uploadedFiles[req.id] && (
              <p className="mt-1 text-xs text-green-600 dark:text-green-400">Uploaded: {uploadedFiles[req.id].fileName}</p>
            )}
          </div>
        ))}
      </div>
      ...
      <div className="flex justify-between pt-6 mt-6 border-t border-neutral-200 dark:border-neutral-700">
        ...
        <button
          disabled={requirements.some((r) => !uploadedFiles[r.id]) || Object.values(uploading).some(Boolean)}
          onClick={onNext}

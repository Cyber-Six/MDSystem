 ...
 * ExpiryWarningBanner
 *
 * Shown inside the chat when the session is expiring within 24 hours.
...
const ExpiryWarningBanner = ({ expiresAt, isExtending, onExtend }) => {
  ...
  // Reset dismiss state when expiresAt changes (i.e. after an extension)
  useEffect(() => {
    setDismissed(false);
  }, [expiresAt]);
  ...
  return (
    <>
      {/* Inline amber banner */}
      <div className="mx-5 mb-3 flex items-center gap-3 px-4 py-2.5 rounded-xl
        ...
        <button
          ...
          className="p-1 rounded text-amber-500 dark:text-amber-400
            ...
            <div className="flex items-start gap-3 p-6 border-b border-neutral-200 dark:border-neutral-700">
              ...
              <div>
                ...
                <p className="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                  Your chat session expires in {timeLeft}.
                </p>
              </div>
              <button
                onClick={() => setShowModal(false)}
                className="ml-auto p-1 rounded-lg hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-colors text-neutral-500"
              >
                <X className="w-4 h-4" />
              </button>
            </div>

 ...
 * Contains all patient-facing routes wrapped in the patient Layout.
 * Handles initial medical record checks for new students.
 */
const PatientApp = () => {
  ...
  useEffect(() => {
    const checkRecordStatus = async () => {
      ...
      if (bypassInitialRecord) {
        console.log('[PatientApp] Bypass enabled - skipping initial record requirement');
      ...
      if (role === 'staff' || role === 'admin') {
        console.log('[PatientApp] Staff/admin user - skipping initial record check');
        setIsCheckingStatus(false);
        return;
      }

      try {
        console.log('[PatientApp] Checking initial record status for student...');
        const { needsInitialRecord, status } = await checkInitialRecordStatus();
        
        console.log('[PatientApp] Initial record check result:', { needsInitialRecord, status });
        ...
        console.error('[PatientApp] Error checking initial record status:', error);

...
function MetricCell({ label, value, highlight = false }) {
  return (
    <div className="px-3 py-2 text-center">
      <p className="text-[10px] text-secondary-400 dark:text-neutral-500 leading-none mb-1">{label}</p>
      {value ? (
        ...
        <p className="text-xs text-secondary-300 dark:text-neutral-600">—</p>
      )}
    </div>
  );
}
...
function FindingTag({ children }) {
  return (
    <span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-medium leading-none bg-warning-50 dark:bg-warning-900/20 text-warning-700 dark:text-warning-400 border border-warning-200 dark:border-warning-800/50">
      {children}
    </span>
  );
}
...
function ToothTag({ children, variant = 'default' }) {
  ...
  return (
    <span className={`inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-mono font-medium leading-none ${styles[variant]}`}>
      {children}
    </span>
  );
}
    ...
    <div className="space-y-2">
      ...
      <PatientSectionCard title="Oral Findings">
        {(dental.oralFindings || []).length === 0 ? (
          <p className="text-[11px] text-secondary-300 dark:text-neutral-600">No oral findings recorded.</p>
        ) : (
      ...
      <PatientSectionCard title="Treatment History">
        {treatments.length === 0 ? (
          <p className="text-[11px] text-secondary-300 dark:text-neutral-600">No dental treatments recorded.</p>
        ) : (
          <div className="divide-y divide-neutral-100 dark:divide-neutral-700/60 -mx-3 -mb-3">
            {treatments.map((t, idx) => (
              <div
                key={idx}
                className="flex items-center justify-between px-3 py-2 hover:bg-neutral-50 dark:hover:bg-neutral-700/30 transition-colors"
              >
                <div>
                  <p className="text-xs font-medium text-secondary-800 dark:text-white leading-none">{t.treatment}</p>
                  <p className="text-[10px] text-secondary-400 dark:text-neutral-500 mt-0.5">{t.dentist}</p>
                </div>
                <span className="text-[10px] text-secondary-400 dark:text-neutral-500 shrink-0 ml-4">{t.date}</span>
              </div>
            ))}
          </div>
        )}
      ...
      {hasChartData && (
        <PatientSectionCard title="Tooth Chart">
          <div className="space-y-2">
            ...
            {chart.missing?.length > 0 && (
              <div>
                <p className="text-[10px] font-medium text-secondary-400 dark:text-neutral-500 uppercase tracking-wider mb-1">Missing</p>
            ...
            {chart.filled?.length > 0 && (
              <div>
                <p className="text-[10px] font-medium text-secondary-400 dark:text-neutral-500 uppercase tracking-wider mb-1">Filled</p>
                <div className="flex flex-wrap gap-1">
            ...
            {chart.decayed?.length > 0 && (
              <div>
                <p className="text-[10px] font-medium text-secondary-400 dark:text-neutral-500 uppercase tracking-wider mb-1">Decayed</p>
                <div className="flex flex-wrap gap-1">
            ...
            {chart.notes && (
              <p className="text-[11px] text-warning-700 dark:text-warning-400 pt-1 border-t border-neutral-100 dark:border-neutral-700/60">
                {chart.notes}
              </p>
            )}

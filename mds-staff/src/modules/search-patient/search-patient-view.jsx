  ...
  const handleKeyDown = useCallback(
    (e) => {
      ...
      if (e.key === 'ArrowDown') {
        ...
        e.preventDefault();
        handleSelectPatient(filtered[focusedIdx]);
      } else if (e.key === 'Escape') {
        setSearchTerm('');
      }
    },
    [filtered, focusedIdx, handleSelectPatient]
  );
        ...
        {tabs.map((tab) => (
          <div
            ...
            onDragOver={(e) => {
              ...
              if (tab.id !== draggedTabId) setDragOverTabId(tab.id);
            }}
            ...
            className={`flex items-center gap-1 pl-3 pr-1 py-2.5 text-sm whitespace-nowrap border-r border-neutral-200 dark:border-neutral-700 transition-colors rounded-t-lg group max-w-[220px] cursor-grab active:cursor-grabbing select-none ${
              activeTabId === tab.id
                ? 'bg-white dark:bg-neutral-800 text-secondary-900 dark:text-white border-t-2 border-t-primary-500'
                : 'text-secondary-500 dark:text-neutral-400 hover:bg-neutral-50 dark:hover:bg-neutral-800/50'
            } ${draggedTabId === tab.id ? 'opacity-40' : ''} ${
              dragOverTabId === tab.id ? 'border-l-2 border-l-primary-400' : ''
            }`}
          >
            <button
